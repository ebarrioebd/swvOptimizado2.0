    ...
    if (metodo === "kriging") {
        ...
        if (dat_semivariograma.m !== "") {
            ...
            wk_kriging.onmessage = (event) => {
                if (event.data.type === "result") { 
                    ...
                    imgZonaAlta = L.imageOverlay(DibujarSuperficieInterpolada(data_ovi_max,cantidad_de_cuadrados_por_ladao, zi, "canvasMapZonaAlta", "imgZonaAlta"), [
                        [caja[0][1], caja[0][0]],
                        [caja[1][1], caja[1][0]]
                    ], {
                        opacity: 1
                    ...
                    var imgk = L.imageOverlay(DibujarSuperficieInterpolada(data_ovi_max,cantidad_de_cuadrados_por_ladao, zi, "canvasMap", ""), [
                        [caja[0][1], caja[0][0]],
                        [caja[1][1], caja[1][0]]
                    ], {
        ...
        wk_idw.onmessage = (event) => {
            if (event.data.type == "result") {
                ...
                imgOpaci = L.imageOverlay(DibujarSuperficieInterpolada(data_ovi_max, cantidad_de_cuadrados_por_ladao, zi, "canvasMap", ""), [
                    [caja[0][1], caja[0][0]],
                    [caja[1][1], caja[1][0]]
                ], {
                    opacity: opacidad_img
...
function generarPI(zonaSelect,metodo) { //genear puntos a interpolar 
    ...
    caja = [[caja[0][0] - aumentC, caja[0][1] - aumentC], [caja[1][0] + aumentC, caja[1][1] + aumentC]];//Escalar Caja
    ...
    if (dA >= dB) { 
        dX =(dA - dB); 
        caja = [[caja[0][0], caja[0][1]], [caja[1][0], caja[1][1] + dX]];
    ...
    if (dB >= dA) { 
        dX =(dB - dA); 
        caja = [[caja[0][0], caja[0][1]], [caja[1][0] + dX, caja[1][1]]];
    ...
    const longitud_de_cuadrado = distance([caja[0][0], caja[0][1]], [caja[1][0], caja[0][1]]); //logitud de lado de la caja
    ...
    puntos_a_interpolar = getPuntosAestimar(caja, tamCuadro, cantidad_de_cuadrados_por_ladao, positions);//parametros(cajaDelimitador,tamanoDeCadaCuadradito,CantidadDeCuadradosDeBase)
